...
style.textContent = `
    ...
    .modal-message { font-size: 18px; margin-bottom: 25px; color: #333; line-height: 1.5; }
    .modal-button {
    ...
    .modal-success .modal-button { background: #28a745; }
    .modal-success .modal-button:hover { background: #218838; }
    .modal-error .modal-button { background: #dc3545; }
    .modal-error .modal-button:hover { background: #c82333; }
    .modal-warning .modal-button { background: #ffc107; color: #333; }
    .modal-warning .modal-button:hover { background: #e0a800; }
`;
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showModal('❌ ' + (data.message || 'Error al restablecer la contraseña'), 'error');
                submitButton.disabled = false;
                submitButton.textContent = originalText;
            }
        } catch (error) {
            console.error('Error:', error);
            showModal('❌ Error de conexión. Intenta de nuevo.', 'error');
            submitButton.disabled = false;
            submitButton.textContent = originalText;
        }
    });
